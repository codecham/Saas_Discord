...
@Controller('/api/discord_V1/')
export class DiscordControllerV1 {
  private readonly logger = new Logger(DiscordControllerV1.name);

  constructor(private readonly discordService: DiscordServiceV1) {}
  ...
  async getAuthenticatedUser(
    ...
    console.log(`Discord access token found: ${discordAccount?.accessToken}`);
  ...
  @UseGuards(JwtAuthGuard)
  @Get('guilds/avaible/list')
  async getUserAvaibleGuildList(
    @Request() req: AuthenticatedRequest,
  ): Promise<DiscordUserGuildListAvaible> {
    this.logger.log(`Getting Discord guilds avaible: ${req.user.email}`);
    ...
    return await this.discordService.getUserAdminGuildAvaible(
      discordAccount.accessToken,
    );
  }
  ...
  getUserInfo(@Request() req: AuthenticatedRequest): any & {
