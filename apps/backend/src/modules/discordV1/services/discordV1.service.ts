...
import type {
  ...
  DiscordGatewayDto,
  DiscordUserGuildListAvaible,
} from '@my-project/shared-types';
import { PrismaService } from 'src/modules/prisma/prisma.service';

@Injectable()
export class DiscordServiceV1 {
  private readonly logger = new Logger(DiscordServiceV1.name);
  ...
  async getUserAdminGuildAvaible(
    userToken: string,
  ): Promise<DiscordUserGuildListAvaible> {
    const adminGuilds = await this.getUserAdminGuilds(userToken);
    ...
    const dbGuildsMap = new Map(dbGuilds.map((g) => [g.guildId, g]));

    const neverAdded: DiscordGuildDto[] = [];
    const inactive: DiscordGuildDto[] = [];
    const active: DiscordGuildDto[] = [];

    for (const discordGuild of adminGuilds) {
      const dbGuild = dbGuildsMap.get(discordGuild.id);

      if (!dbGuild) {
        // Bot jamais ajouté sur ce serveur
        neverAdded.push(discordGuild);
      } else if (!dbGuild.isActive) {
        // Bot a été présent mais ne l'est plus
        inactive.push(discordGuild);
      } else {
        // Bot actuellement présent et actif
        active.push(discordGuild);
      }
    }
    return {
      neverAdded,
