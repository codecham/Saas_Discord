...
import { BotEventDto, EventType, ReactionAddEventData } from '@my-project/shared-types';
...
export class MessageReactionAddListener extends Listener {
  ...
  public override async run(
    ...
    if (!isListenerEnabled('MESSAGE_REACTION_ADD')) {
      return;
    }
    ...
    if (reaction.partial) {
      try {
        ...
        this.container.logger.warn(`[REACTION_ADD] Failed to fetch reaction`);
        return;
      }
    }
    ...
    if (user.partial) {
      try {
        ...
        this.container.logger.warn(`[REACTION_ADD] Failed to fetch user`);
        return;
      }
    }
    ...
    this.container.logger.debug(
      `[REACTION_ADD] Guild: ${reaction.message.guildId} | User: ${user.id} | Emoji: ${reaction.emoji.name}`
    );
  }

  private extractReactionData(reaction: MessageReaction, user: User): ReactionAddEventData {
    return {
      emoji: {
